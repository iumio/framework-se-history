...
class AssetsManager extends ToolsManager implements ModuleManager
{
    ...
    public function clearAssets(array $options)
    {
        ...
        if (!is_dir(FEnvFcm::get("framework.root")."/public/components/apps/dev/".strtolower($appname)) ||
            !is_dir(FEnvFcm::get("framework.root")."/public/components/apps/prod/".strtolower($appname))) {
            if (!in_array("--quiet", $options)) {
