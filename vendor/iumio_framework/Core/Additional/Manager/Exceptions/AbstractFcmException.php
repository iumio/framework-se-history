 ...
 */
abstract class AbstractFcmException extends \Exception implements ExceptionFcmInterface
{
    protected $code = 0;
    ...
    public function __construct(ArrayObject $component, string $header_message)
    {
        ...
        foreach ($it as $one => $value) {
            if ($it->key() == "codeTitle") {
                ...
                $this->explain = trim(preg_replace('/\s\s+/', ' ', strip_tags($value)));
            } elseif ($it->key() == "solution") {
                $this->solution = trim(preg_replace('/\s\s+/', ' ', strip_tags ($value)));
            } elseif ($it->key() == "inlog") {
                $this->inlog = $value;
    ...
    public function display(string $code, string $message)
    {
        ...
        Output::displayAsError("An exception was generated\n\nUidie : ".
            $this->uidie."\nTime : ". $this->time."\nType : ".
    ...
    final protected function writeFileError():int
    {
        ...
        $debug['env'] = $d1.FEnvFcm::get("framework.env").$d2;
        $debug['method'] = $d1."/".$d2;
        if ($this->trace != null) {
        ...
        $f->open(FEnvFcm::get("framework.logs").
            strtolower(FEnvFcm::get("framework.env")).".log", "a+", true);
        $f->put($strlog);
        $f->close();
        return (1);
    }
