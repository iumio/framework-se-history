    ...
    public function clear(string $appname, string $env):int
    {
        ...
        if ($appname != "" && in_array(strtolower($env), array("dev", "prod", "all"))) {
            if (in_array(strtolower($env), array("dev", "prod"))) {
                ...
                Server::delete(
                    ROOT_WEB_ASSETS.strtolower("prod")."/".strtolower($appname),
                    'directory'
                );
            }
        }
        ...
        foreach ($apps as $app => $val) {
            ...
            if ($hassets == 1) {
                ...
                foreach ($webapp as $key => $value) {
                    $webapp[$key] = str_replace(ROOT . "/apps/" . ($appname) .
            ...
            if ($dev_hassets == 1 && $hassets == 1) {
                ...
                foreach ($webdev as $key => $value) {
                    $webdev[$key] = str_replace(ROOT . "/web/components/apps/dev/" .
                ...
                if ($dev_hassets == 1 && $status_dev == 1 && $hassets == 1) {
                    for ($i = 0; $i < count($webapp); $i++) {
                        if (!file_exists(ROOT . "/web/components/apps/dev/" . strtolower($appname) .
                            "/" . $webdev[$i])) {
                            $status_dev = 0;
                            break;
                        }
                        $def = $diff::compareFiles(ROOT . "/web/components/apps/dev/" . strtolower($appname) .
                            "/" . $webdev[$i], ROOT . "/apps/" . ($appname) . "/Front/Resources/" . $webapp[$i]);
            ...
            if ($prod_hassets == 1 &&  $hassets == 1) {
                ...
                foreach ($webprod as $key => $value) {
                    $webprod[$key] = str_replace(ROOT .
                        "/web/components/apps/prod/" . strtolower($appname), "", $value);
                }
                ...
                if ($prod_hassets == 1 && $status_prod == 1) {
                    for ($i = 0; $i < count($webapp); $i++) {
                        if (!file_exists(ROOT . "/web/components/apps/prod/" .
                            strtolower($appname) . "/" . $webprod[$i])) {
                            $status_dev = 0;
                            break;
                        }
                        $def = $diff::compareFiles(ROOT . "/web/components/apps/prod/" . strtolower($appname) .
                            "/" . $webprod[$i], ROOT . "/apps/" .
