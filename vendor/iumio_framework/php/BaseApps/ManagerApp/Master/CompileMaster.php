        ...
        foreach ($iterator as $dir_info) {
            if ($dir_info->isDir() && !$dir_info->isDot()) {
                ...
                array_push($directory, array("path" => $dir_info->getRealPath(), "name" => $dir_info->getFilename(),
                    ...
                    "perms" => $perms, "status" => (($this->checkFolderIsEmptyOrNot(ROOT_COMPILED.$dir_info->getFilename()) == true)? "Empty" : "Is not empty"),
                    "env" => $dir_info->getFilename(),
                    "clear" => $this->generateRoute("iumio_manager_compile_manager_remove", array("env" => $dir_info->getFilename()), null, true)));
            }
        }
