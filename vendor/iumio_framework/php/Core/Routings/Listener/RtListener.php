...
class RtListener implements Listener
{
    ...
    public function open(bool $isbase = false):int
    {
        ...
        foreach ($this->routers as $file) {
            $scope = null;
            if (($router = fopen(((!$isbase) ? ROOT . "/apps/" : BASE_APPS) . $this->appName .
                "/Routing/" . $file, "r"))) {
                if ($this->analyseRT($router, $file, $this->appName) == 0) {
                    exit();
                }
                rewind($router);
                ...
                while ($listen = fgets($router, 1024)) {
                    ...
                    if ($start === 1 && $end === 1) {
                        if ($this->checkIfKeyExist($rtarray, $file, $this->appName) != 1) {
                            exit();
                        }
                        $rtarray = array("method" => "", "path" => "", "name" => "", "visibility" =>
        ...
        for ($i = 0; $i < count($routingArray); $i++) {
            ...
            if (count($method) == 2) {
                ...
                if (!method_exists($reflect->newInstance(), $function."Activity") ||
                    !is_callable(array($reflect->newInstance(), $function."Activity"))) {
                    throw new Server500(new \ArrayObject(array("explain" => "Activity is not callable : '".
                        $controller.":".$function."Activity"."' : ".$this->appName, "solution" =>
    ...
    private function checkIfKeyExist(array $resource, string $filename, string $appname):int
    {
        ...
        if (isset($resource["visibility"]) && !in_array(
            $resource["visibility"],
            array("public", "private", "disabled")
        )) {
            throw new  Server500(new \ArrayObject(array("explain" =>
    ...
    public function listingRouters(bool $isbase = false):int
    {
        if ($this->appWording() == 1) {
            $this->routers = scandir((($isbase == false)? ROOT . "/apps/" : BASE_APPS) .
        ...
        if ($start != $end) {
            if ($start < $end) {
                throw new Server500(new \ArrayObject(array("explain" => "Missing tag 'route' in "
                    .$filename. " Routing file : ".$appname , "solution" => "Please check your RT file")));
            }
            if ($end < $start) {
