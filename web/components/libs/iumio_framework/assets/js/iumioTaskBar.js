    ...
    xhr.addEventListener('readystatechange', function() {
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            ...
            if (ObjectLength(result) > 0)
            {
                ...
                {
                    var e =   '<li class="iumioTaskBarOneApp" '+((result[i]['enabled'] === "yes")? "  style='background-color:rgba(23, 202, 23, 0.45);' " : "   style='rgba(236, 40, 40, 0.45);' ")+'  attr-href="'+result[i]['link_auto_dis_ena']+'">'+result[i]['name'] +" "+((result[i]['enabled'] === "yes")? " - Enabled" : "- Disabled") +'</li>';
                    str += e;
                }
                str += '';
