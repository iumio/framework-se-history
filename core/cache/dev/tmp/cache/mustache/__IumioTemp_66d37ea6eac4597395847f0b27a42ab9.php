...
class __IumioTemp_66d37ea6eac4597395847f0b27a42ab9 extends Mustache_Template
{
...
';
        ...
        $value = $this->resolveValue($context->find('css'), $context);
        $filter = $context->find('bootstrap(\'css\', \'min\')');
        if (!(is_object($filter) && is_callable($filter))) {
            throw new Mustache_Exception_UnknownFilterException('bootstrap(\'css\', \'min\')');
        }
